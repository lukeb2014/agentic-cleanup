...
mkdir -p "$MOCK_DIR/.local/share/agentic-cleanup/data"
mkdir -p "$MOCK_DIR/.local/share/agentic-cleanup/context"

bash "$BASE_DIR/checks/disk.sh" 2>/dev/null || true
assert_file_exists "disk baseline created" "$MOCK_DIR/.local/share/agentic-cleanup/data/disk-baseline"

BASELINE=$(cat "$MOCK_DIR/.local/share/agentic-cleanup/data/disk-baseline")
...
bash "$BASE_DIR/checks/disk.sh" 2>/dev/null || true
DISK_CONTEXT=$(cat "$MOCK_DIR/.local/share/agentic-cleanup/context/disk.md" 2>/dev/null || echo "")
...
assert_file_exists "ros2 processes file created" "$MOCK_DIR/.local/share/agentic-cleanup/data/ros2-processes"

echo ""
echo "Test: ros2.sh produces no context on second run if processes unchanged"
bash "$BASE_DIR/checks/ros2.sh" 2>/dev/null || true
ROS2_CONTEXT=$(cat "$MOCK_DIR/.local/share/agentic-cleanup/context/ros2.md" 2>/dev/null || echo "")
